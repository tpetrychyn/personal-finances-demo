...
|---|---|
| `index.html` | Page markup only — the cards, plus `<link>`/`<script>` tags |
| `styles.css` | All styling (design tokens in `:root`, then component rules) |
| `js/state.js` | `DEFAULT_STATE`, derived getters, undo, persistence + schema migration |
...
model the old single-file version used). `js/main.js` must stay last — it runs the init.
